...
__all__ = ['IndicatorManageController']


class IndicatorManageController(BaseController):
    def __init__(self):
        self.util = Utility();
        
    
    @expose('comcenter.templates.indicator.index')
    def index(self):
        reload(sys);
    ...
    @expose('comcenter.templates.indicator.add')
    def add(self):
        reload(sys);
        sys.setdefaultencoding("utf-8");
        set_lang("th"); 
    ...
    @expose('comcenter.templates.indicator.report1')
    def report1(self,**kw):
        reload(sys);
        sys.setdefaultencoding("utf-8");
        set_lang("th");
        ...
        self.listTeam = RiskSection.listAll();
        self.listTeamCrom = [];
        ...
        listHash =  IndicatorsService.listReport1Indicator(self.team_id,year);
    ...
    @expose('comcenter.templates.indicator.report2')
    def report2(self,**kw):
        reload(sys);
        sys.setdefaultencoding("utf-8");
        set_lang("th");
        ...
        listHash =  IndicatorsService.listReport2Indicator(year);
       
        #log_view_report
        self.saveLogView();
         
        return dict(page='indicator',util=self.util,year=year ,listYear = listYear,  listHash=listHash  );   
        #return dict(page='risk',util=self.util,year=year,month=month,listMonth=listMonth,listYear = listYear,section=section,pro_clinic=pro_clinic ,pro_physic=pro_physic,priority10=priority10,priority10inphysic = priority10InPhysic,startDate = startDate,stopDate= stopDate);
    ...
    def updateIndicator(self, **kw):
        ...
        for value in data:
            ...
            if(len(indicators_service) ==0):
                ...
                indicatorsService = IndicatorsService.getById(indicators_service);
                indicatorsService.indicator_value = indicator_value;
    ...
    def listIndicatorBySection(self,**kw):
        ...
        self.List = IndicatorsService.listIndicatorBySection(sectionid , years_id,start_year, months_id);
